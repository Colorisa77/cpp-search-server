...
public:
    ...
    void AddDocument(int document_id, const string& document, DocumentStatus status, const vector<int>& ratings) {
        if (document_id < 0 || (documents_.count(document_id) > 0)) {
            throw invalid_argument("SearchServer::AddDocument, invalid document id");
        }
    ...
    vector<Document> FindTopDocuments(const string& raw_query) const { return FindTopDocuments(raw_query, DocumentStatus::ACTUAL); }
...
int main() {
    SearchServer search_server("�� ��\x02�� ��"s);
